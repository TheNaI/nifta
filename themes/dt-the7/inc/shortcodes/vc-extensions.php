...
class The7_Inc_Shortcodes_VCParams {
	...
	public function taxonomy_settings_field( $settings, $value ) {
		...
		return '<div class="dt_taxonomy_block">'
		       .'<input type="hidden" name="'.$settings['param_name'].'" class="wpb_vc_param_value wpb-checkboxes '.$settings['param_name'].' '.$settings['type'].'_field" value="'.$value_inner.'" />'
	...
	public function register_params() {
		...
		$this->compatible_vc_add_shortcode_param( 'dt_taxonomy', array( $this, 'taxonomy_settings_field' ), $script_url );
		$this->compatible_vc_add_shortcode_param( 'dt_posttype', array( $this, 'posttype_settings_field' ), $script_url );
		$this->compatible_vc_add_shortcode_param( 'dt_spacing', array( $this, 'spacing_param' ), $script_url );
		$this->compatible_vc_add_shortcode_param( 'dt_responsive_columns', array( $this, 'responsive_columns_param' ), $script_url );
		$this->compatible_vc_add_shortcode_param( 'dt_dimensions', array( $this, 'dimensions_param' ), $script_url );
		$this->compatible_vc_add_shortcode_param( 'dt_number', array( $this, 'number_param' ), $script_url );
		$this->compatible_vc_add_shortcode_param( 'dt_font_style', array( $this, 'font_style_param' ), $script_url );
		$this->compatible_vc_add_shortcode_param( 'dt_switch', array( $this, 'switch_param' ), $script_url );
		$this->compatible_vc_add_shortcode_param( 'dt_navigation' , array($this, 'dt_icon_settings_field' ), $script_url );
		$this->compatible_vc_add_shortcode_param( 'dt_soc_icon_manager' , array($this, 'dt_soc_icons_settings_field' ), $script_url );

		// Extend default autocomplete param with custom js.
		$this->compatible_vc_add_shortcode_param( 'autocomplete', 'vc_autocomplete_form_field', "{$dir}/inc/shortcodes/vc_extend/dt-autocomplete.js" );
	}
	...
	public function dt_soc_icons_settings_field($settings, $soc_value)
		{
			...
			if($param_name == "dt_soc_icon"){
				$icons = array('dt-icon-px-500','dt-icon-behance','dt-icon-blogger','dt-icon-delicious','dt-icon-devian','dt-icon-dribbble','dt-icon-facebook','dt-icon-flickr','dt-icon-foursquare','dt-icon-github','dt-icon-google','dt-icon-instagram','dt-icon-lastfm','dt-icon-linkedin','dt-icon-mail','dt-icon-odnoklassniki','dt-icon-pinterest','dt-icon-reddit','dt-icon-research-gate','dt-icon-rss','dt-icon-skype','dt-icon-soundcloud','dt-icon-stumbleupon','dt-icon-tripedvisor','dt-icon-tumbler','dt-icon-twitter','dt-icon-viber','dt-icon-vimeo','dt-icon-vk','dt-icon-website','dt-icon-weibo','dt-icon-whatsapp','dt-icon-xing','dt-icon-yelp','dt-icon-you-tube');
			}
