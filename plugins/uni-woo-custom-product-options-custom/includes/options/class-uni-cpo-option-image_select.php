...
/*
*   Uni_Cpo_Option_image_select class
*
*/

class Uni_Cpo_Option_image_select extends Uni_Cpo_Option {

    public $id = 0;

    public $post = null;

    public $option_type = 'image_select';
	...
	public function __construct( $Option = false ) {
        ...
        $this->option_icon = 'fa-picture-o';
        $this->option_name = esc_html__('Image select', 'uni-cpo');
        $this->tab_settings = apply_filters( "uni_cpo_".$this->option_type."_admin_settings_filter", array(
                                    array(
                                        ...
                                        'settings' => array('field_slug', 'field_required_enable',
                                            ...
                                            'field_image_change_disable',
                                            'field_option_image_select_options')
                                    ),
                                    array(
        ...
        $this->specific_settings = apply_filters( "uni_cpo_".$this->option_type."_admin_specific_settings_filter", array(
                                    'field_option_image_select_options' => array( 'image_select' =>
                                        array(
                                            ...
                                            'name' => 'field_option_image_select_options',
                                            'required' => false
                                        )
                                    )
                                )
	...
	public function render_option( $aChildrenOptionsIds = array() ) {
        ...
        $cpo_general_settings   = get_option( 'uni_cpo_settings_general', UniCpo()->default_settings() );
        $sMainImageSize         = $cpo_general_settings['product_image_size'];
        ...
        if ( isset( $option_meta['_uni_cpo_field_image_change_disable'][0] ) && $option_meta['_uni_cpo_field_image_change_disable'][0] === 'yes' ) {
        ...
        echo '<div id="'.esc_attr( $sElementFieldName ).'" data-type="' . esc_attr( $this->get_type() ) . '" class="uni_cpo_fields_container uni_cpo_field_type_'.esc_attr( $this->get_type() ) . ( (!empty($option_meta['_uni_cpo_field_extra_class'][0])) ? ' '.esc_attr( $option_meta['_uni_cpo_field_extra_class'][0] ) : '' ) .'">';
            ...
            $sSerializedValues = ( !empty($option_meta['_uni_cpo_field_option_image_select_options'][0]) && $option_meta['_uni_cpo_field_option_image_select_options'][0] ) ? $option_meta['_uni_cpo_field_option_image_select_options'][0] : '';
                ...
                if ( !empty( $aArrayOfValues ) && is_array( $aArrayOfValues ) ) {
                    ...
                    foreach ( $aArrayOfValues as $sKey => $aValue ) {
                        ...
                        if ( !empty($aValue['image_id']) ) {
                            ...
                            if ( $iImageCustomSize > 200 ) {
                                $aImageThumb = wp_get_attachment_image_src( $iThumbId, array($iImageCustomSize, $iImageCustomSize) );
                            } else {
                                $aImageThumb = wp_get_attachment_image_src( $iThumbId, 'uni-cpo-thumb' );
                            }

                            if ( ! empty( $sSelectValue ) ) {
                                echo '<option data-thumbimageuri="'.esc_attr($aImageFull[0]).'" data-imagetitle="'.$sThisOptionLabel.'" data-fullimageuri="'.esc_attr($aImageFull[0]).'" data-imageuri="'.esc_attr($aImageFull[0]).'" data-imagesrcset="'.esc_attr($aImageFull[0]).'" data-imagesizes="'.esc_attr($sImageSizes).'"'.selected( $aValue['slug'], $sSelectValue, false ).' value="'.esc_attr($aValue['slug']).'">';
                            } else {
                                echo '<option data-thumbimageuri="'.esc_attr($aImageFull[0]).'" data-imagetitle="'.$sThisOptionLabel.'" data-fullimageuri="'.esc_attr($aImageFull[0]).'" data-imageuri="'.esc_attr($aImageFull[0]).'" data-imagesrcset="'.esc_attr($aImageFull[0]).'" data-imagesizes="'.esc_attr($sImageSizes).'"'.selected( $i, ( isset($aValue['default']) ) ? $aValue['default'] : '', false ).' value="'.esc_attr($aValue['slug']).'">';
                            }
                            echo '</option>';
        ...
        jQuery( document ).ready( function( $ ) {
            ...
            var $image_select = $('[name=<?php echo esc_attr( $sElementFieldName ) ?>]');
            uniCpoTransformImageSelect( $image_select );
	...
	public function get_order_form_field( $posted_data = array() ) {
        $option_meta    = $this->get_post_meta();
        $subptions = ( ! empty( $option_meta['_uni_cpo_field_option_image_select_options'][0] ) && $option_meta['_uni_cpo_field_option_image_select_options'][0] ) ? $option_meta['_uni_cpo_field_option_image_select_options'][0] : '';
	...
	public function calculation( $aFormPostData, $bCartMeta = false ) {
        ...
        if ( isset($aFormPostData[$sElementFieldName]) && !empty($aFormPostData[$sElementFieldName]) ) {
            if ( get_post_meta( $this->get_id(), '_uni_cpo_field_option_image_select_options', true ) ) {
                $aSelectOptions = get_post_meta( $this->get_id(), '_uni_cpo_field_option_image_select_options', true );
                $sAllOptionsChosen = '';
                if ( !empty($aSelectOptions) ) {
                unset($aSelectOptions['default']);
                foreach ( $aSelectOptions as $sKey => $aValue ) {
