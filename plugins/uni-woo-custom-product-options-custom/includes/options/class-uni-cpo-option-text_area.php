...
class Uni_Cpo_Option_text_area extends Uni_Cpo_Option {

    public $id = 0;

    public $post = null;

    public $option_type = 'text_area';
	...
	public function __construct( $Option = false ) {
        ...
        $this->option_icon = 'fa-terminal';
        $this->option_name = esc_html__('Textarea', 'uni-cpo');
        $this->tab_settings = apply_filters( "uni_cpo_".$this->option_type."_admin_settings_filter", array(
                                    array(
                                        ...
                                        'settings' => array('field_slug', 'field_required_enable',
                                            'field_chars_min', 'field_chars_max')
                                    ),
                                    array(
	...
	public function render_option( $aChildrenOptionsIds = array() ) {
        ...
        echo '<div id="'.esc_attr( $sElementFieldName ).'" data-type="' . esc_attr( $this->get_type() ) . '" class="uni_cpo_fields_container uni_cpo_field_type_'.esc_attr( $this->get_type() ) . ( (!empty($aOptionCustomMeta['_uni_cpo_field_extra_class'][0])) ? ' '.esc_attr( $aOptionCustomMeta['_uni_cpo_field_extra_class'][0] ) : '' ) .'">';
            ...
            if ( !empty($aOptionCustomMeta['_uni_cpo_field_required_enable'][0]) && $aOptionCustomMeta['_uni_cpo_field_required_enable'][0] == 'yes' ) {
                echo '<textarea id="'.esc_attr( $sElementFieldName ).'-field" name="'.esc_attr( $sElementFieldName ).'" class="'.esc_attr( $sElementFieldName ).'-field js-uni-cpo-field-'.esc_attr( $this->get_type() ).' uni-cpo-required" data-parsley-required="true" data-parsley-trigger="change focusout submit"'.( (isset($aOptionCustomMeta['_uni_cpo_field_chars_min'][0]) && !empty($aOptionCustomMeta['_uni_cpo_field_chars_min'][0])) ? ' data-parsley-minlength="'.esc_attr($aOptionCustomMeta['_uni_cpo_field_chars_min'][0]).'"' : '' ).''.( (isset($aOptionCustomMeta['_uni_cpo_field_chars_max'][0]) && !empty($aOptionCustomMeta['_uni_cpo_field_chars_max'][0])) ? ' data-parsley-maxlength="'.esc_attr($aOptionCustomMeta['_uni_cpo_field_chars_max'][0]).'"' : '' ).'>'.$sInputValue.'</textarea>';
            } else {
                echo '<textarea id="'.esc_attr( $sElementFieldName ).'-field" name="'.esc_attr( $sElementFieldName ).'" class="'.esc_attr( $sElementFieldName ).'-field js-uni-cpo-field-'.esc_attr( $this->get_type() ).'"'.( (isset($aOptionCustomMeta['_uni_cpo_field_chars_min'][0]) && !empty($aOptionCustomMeta['_uni_cpo_field_chars_min'][0])) ? ' data-parsley-minlength="'.esc_attr($aOptionCustomMeta['_uni_cpo_field_chars_min'][0]).'"' : '' ).''.( (isset($aOptionCustomMeta['_uni_cpo_field_chars_max'][0]) && !empty($aOptionCustomMeta['_uni_cpo_field_chars_max'][0])) ? ' data-parsley-maxlength="'.esc_attr($aOptionCustomMeta['_uni_cpo_field_chars_max'][0]).'"' : '' ).''.( (isset($aOptionCustomMeta['_uni_cpo_field_chars_min'][0]) || isset($aOptionCustomMeta['_uni_cpo_field_chars_max'][0])) ? ' data-parsley-trigger="change focusout submit"' : '' ).'>'.$sInputValue.'</textarea>';
            }
