...
/*
*   Uni_Cpo_Option_notice class
*
*/

class Uni_Cpo_Option_notice extends Uni_Cpo_Option {

    public $id = 0;

    public $post = null;

    public $option_type = 'notice';
	...
	public function __construct( $Option = false ) {
        ...
        $this->option_icon = 'fa-sticky-note-o';
        $this->option_name = esc_html__('Notice', 'uni-cpo');
        $this->tab_settings = apply_filters( "uni_cpo_".$this->option_type."_admin_settings_filter", array(
                                    ...
                                    array(
                                        ...
                                        'settings' => array('field_notice_type', 'field_notice_header',
                                        'field_notice_text', 'field_notice_shortcode', 'field_extra_class')
                                    ),
                                    array(
	...
	public function render_option( $aChildrenOptionsIds = array() ) {
        ...
        echo '<div id="'.esc_attr( $sElementFieldName ).'" data-type="' . esc_attr( $this->get_type() ) . '" class="uni_cpo_fields_container uni_cpo_field_type_' . esc_attr( $this->get_type() ) . ' uni-cpo-notice-type-'.( (isset($aOptionCustomMeta['_uni_cpo_field_notice_type'][0]) && !empty($aOptionCustomMeta['_uni_cpo_field_notice_type'][0])) ? esc_attr( $aOptionCustomMeta['_uni_cpo_field_notice_type'][0] ) : 'style-one' ) . ( (!empty($aOptionCustomMeta['_uni_cpo_field_extra_class'][0])) ? ' '.esc_attr( $aOptionCustomMeta['_uni_cpo_field_extra_class'][0] ) : '' ) . '">';
            ...
            if ( isset($aOptionCustomMeta['_uni_cpo_field_notice_text'][0]) && !empty($aOptionCustomMeta['_uni_cpo_field_notice_text'][0]) ) {
                echo '<div>'.wp_kses( $aOptionCustomMeta['_uni_cpo_field_notice_text'][0], $aAllowedTootltipHtml ).'</div>';
            }
            if ( isset($aOptionCustomMeta['_uni_cpo_field_notice_shortcode'][0]) && !empty($aOptionCustomMeta['_uni_cpo_field_notice_shortcode'][0]) ) {
                echo '<div class="uni-cpo-shortcode-container">'.do_shortcode( stripslashes_deep( $aOptionCustomMeta['_uni_cpo_field_notice_shortcode'][0], $aAllowedHtml ) ).'</div>';
            }
