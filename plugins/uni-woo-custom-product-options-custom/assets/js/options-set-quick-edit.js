...
inlineEditPost = {
	...
	setBulk : function(){
		...
		$( 'tbody th.check-column input[type="checkbox"]' ).each( function() {
			if ( $(this).prop('checked') ) {
				...
				theTitle = $('#inline_'+id+' .post_title').html() || uniInlineEditL10n.notitle;
				te += '<div id="ttle'+id+'"><a id="_'+id+'" class="ntdelbutton" title="'+uniInlineEditL10n.ntdeltitle+'">X</a>'+theTitle+'</div>';
			}
		});
		...
		if ( 'post' === type ) {
			...
			tax = 'post_tag';
			$('tr.inline-editor textarea[name="tax_input['+tax+']"]').suggest( ajaxurl + '?action=ajax-tag-search&tax=' + tax, { delay: 500, minchars: 2, multiple: true, multipleSep: uniInlineEditL10n.comma } );
		}
	...
	edit : function(id) {
		...
		$('.tags_input', rowData).each(function(){
			...
			textarea.suggest( ajaxurl + '?action=ajax-tag-search&tax=' + taxname, { delay: 500, minchars: 2, multiple: true, multipleSep: uniInlineEditL10n.comma } );
		});
				...
				if (r) {
					if ( -1 !== r.indexOf( '<tr' ) ) {
						...
						$( inlineEditPost.what + id ).hide().fadeIn( 400, function() {
							...
							wp.a11y.speak( uniInlineEditL10n.saved );
						});
					} else {
						r = r.replace( /<.[^<>]*?>/g, '' );
					...
					$errorSpan.html( uniInlineEditL10n.error ).show();
					wp.a11y.speak( uniInlineEditL10n.error );
				}
			},
		'html');
